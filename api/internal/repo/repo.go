...
func (r *Repo) confirmRoboEntrega(ctx context.Context, tx pgx.Tx, up *models.UploadPendente, userID uuid.UUID, resp *models.ConfirmarUploadResponse) error {
	...
	// Notificação pra o cliente (PWA) — esteira robô→cliente automática.
	// Falha aqui não derruba o upload: registra o erro mas segue. A notificação
	// é um nice-to-have, não bloqueia a entrega que já foi criada.
	if err := criarNotificacaoNovaGuia(ctx, tx, up.OrgID, empresaID, obrigacaoID, entregaID, arquivoID, competencia); err != nil {
		// Log silencioso — em prod entra no zerolog do middleware via ctx
		_ = err
	}
...
// criarNotificacaoNovaGuia gera 1 cliente_notificacoes do tipo 'nova_guia'.
// Best-effort: erro de SELECT/INSERT não estoura — registra mas não derruba a transação.
// Título: "Nova guia: {nome da obrigação}". Corpo: "Referente a {competencia}".
func criarNotificacaoNovaGuia(
	ctx context.Context,
	tx pgx.Tx,
	orgID, empresaID, obrigacaoID, entregaID, arquivoID uuid.UUID,
	competencia string,
) error {
	var nome, codigo string
	if err := tx.QueryRow(ctx, `
		SELECT nome, codigo FROM public.obrigacoes_catalogo WHERE id = $1
	`, obrigacaoID).Scan(&nome, &codigo); err != nil {
		// Sem nome — usa fallback genérico
		nome = "Documento"
		codigo = ""
	}

	titulo := fmt.Sprintf("Nova guia: %s", nome)
	corpo := fmt.Sprintf("Referente a %s. Disponível para download no portal.", formatarCompetenciaBR(competencia))

	payload, err := json.Marshal(map[string]any{
		"entrega_id":       entregaID.String(),
		"arquivo_id":       arquivoID.String(),
		"obrigacao_id":     obrigacaoID.String(),
		"obrigacao_codigo": codigo,
		"obrigacao_nome":   nome,
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO public.cliente_notificacoes
		    (org_id, empresa_id, tipo, titulo, corpo, payload)
		VALUES ($1, $2, 'nova_guia'::app.cliente_notificacao_tipo, $3, $4, $5::jsonb)
	`, orgID, empresaID, titulo, corpo, payload)
	return err
}

// formatarCompetenciaBR — "2026-05" → "Maio/2026". Tolerante a entrada malformada.
func formatarCompetenciaBR(yyyymm string) string {
	if len(yyyymm) != 7 || yyyymm[4] != '-' {
		return yyyymm
	}
	meses := []string{"Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"}
	var mm int
	if _, err := fmt.Sscanf(yyyymm[5:7], "%d", &mm); err != nil || mm < 1 || mm > 12 {
		return yyyymm
	}
	return fmt.Sprintf("%s/%s", meses[mm-1], yyyymm[:4])
}
