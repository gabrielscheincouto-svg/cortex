...
// ─── Balancete Findings (auditoria 046) ───────────────────────────────────
// Achados produzidos pela auditoria sobre balancete_contas + comparação com
// histórico. Cada finding precisa ser tratado (justificado/corrigido/arquivado)
// antes do mês ser liquidado.
...
const (
	FindingStatusAberto      FindingStatus = "aberto"
	FindingStatusJustificado FindingStatus = "justificado"
	FindingStatusCorrigido   FindingStatus = "corrigido"
	FindingStatusArquivado   FindingStatus = "arquivado"
)

type FindingTipo string

const (
	FindingTipoSaldoInvertido     FindingTipo = "saldo_invertido"
	FindingTipoVariacaoExtrema    FindingTipo = "variacao_extrema"
	FindingTipoContaReativada     FindingTipo = "conta_reativada"
	FindingTipoOutlierEstatistico FindingTipo = "outlier_estatistico"
	FindingTipoReceitaZerada      FindingTipo = "receita_zerada"
	FindingTipoDespesaAtipica     FindingTipo = "despesa_atipica"
	FindingTipoBateDREDivergente  FindingTipo = "bate_dre_divergente"
)

type BalanceteFinding struct {
	ID              uuid.UUID       `json:"id"`
	OrgID           uuid.UUID       `json:"org_id"`
	BalanceteID     uuid.UUID       `json:"balancete_id"`
	Severity        FindingSeverity `json:"severity"`
	Tipo            FindingTipo     `json:"tipo"`
	Titulo          string          `json:"titulo"`
	Descricao       string          `json:"descricao"`
	ContaCodigo     *string         `json:"conta_codigo,omitempty"`
	ContaDescricao  *string         `json:"conta_descricao,omitempty"`
	ValorReferencia *float64        `json:"valor_referencia,omitempty"`
	Contexto        []byte          `json:"contexto,omitempty"` // raw jsonb
	Status          FindingStatus   `json:"status"`
	Justificativa   *string         `json:"justificativa,omitempty"`
	ResolvidoPorID  *uuid.UUID      `json:"resolvido_por_id,omitempty"`
	ResolvidoEm     *time.Time      `json:"resolvido_em,omitempty"`
	RunToken        *uuid.UUID      `json:"run_token,omitempty"`
	CreatedAt       time.Time       `json:"created_at"`
	UpdatedAt       time.Time       `json:"updated_at"`
}

// RodarAuditoriaResult — resumo retornado pelo POST /balancetes/:id/auditoria/rodar.
type RodarAuditoriaResult struct {
	BalanceteID   uuid.UUID      `json:"balancete_id"`
	RunToken      uuid.UUID      `json:"run_token"`
	FindingsCount int            `json:"findings_count"`
	PorSeveridade map[string]int `json:"por_severidade"`
	PorTipo       map[string]int `json:"por_tipo"`
}

// ResolverFindingDTO — payload do PATCH /findings/:id pra justificar/marcar resolvido.
// Status precisa estar em {justificado, corrigido, arquivado}. Justificativa é
// obrigatória para 'justificado' e 'arquivado'.
type ResolverFindingDTO struct {
	Status        FindingStatus `json:"status"`
	Justificativa *string       `json:"justificativa,omitempty"`
}

type BalanceteFindingsResumo struct {
