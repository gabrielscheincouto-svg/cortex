...
func New(cfg *config.Config, r *repo.Repo, hub *realtime.Hub) *fiber.App {
	...
	// Rotas autenticadas
	api := app.Group("/api/v1", auth.Middleware(cfg.SupabaseJWTSecret, cfg.SupabaseURL, r.GetCurrentOrgID, true, false))
	...
	// Auditoria contábil (migration 046) — 5 regras + bate-DRE → findings → justificar → liquidar
	api.Post("/balancetes/:id/auditoria/rodar", h.RodarAuditoriaBalancete)
	api.Get("/balancetes/:id/findings", h.ListBalanceteFindings)
	api.Get("/balancetes/:id/findings/resumo", h.GetBalanceteFindingsResumo)
	api.Post("/balancetes/:id/liquidar", h.LiquidarBalancete)
	api.Patch("/findings/:id", h.ResolverBalanceteFinding)
	...
	app.Get("/ws", auth.Middleware(cfg.SupabaseJWTSecret, cfg.SupabaseURL, r.GetCurrentOrgID, true, true), realtime.WSUpgrade(hub))
