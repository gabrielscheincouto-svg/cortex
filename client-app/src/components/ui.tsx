/**
 * Primitivos de UI — espelho dos componentes do `web/components/ui.tsx`.
 * Mantém o mesmo design system para consistência visual entre escritório e cliente.
 */
import {
  forwardRef,
  type ReactNode,
...
export function Card({ className, children }: { className?: string; children: ReactNode }) {
  return (
    <div className={cn('rounded-xl border border-black/10 bg-white p-5', className)}>{children}</div>
  )
}
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  { className, variant = 'secondary', icon: Icon, size = 'md', children, ...props },
  ref,
) {
  return (
    <button
...
export function Avatar({
  ...
  return (
    <span
      className={cn(dim, 'inline-flex shrink-0 items-center justify-center rounded-full font-semibold')}
...
export function Spinner({ size = 16 }: { size?: number }) {
  return (
    <span
      ...
      className="inline-block animate-spin rounded-full border-2 border-ink-200 border-t-ink-700"
