...
export function apiBrowser(token: string) {
  return {
    ...
    // ─── Auditoria do balancete (migration 046) ───────────────────────────
    rodarAuditoriaBalancete: (balanceteId: string) =>
      fetchJSON<RodarAuditoriaResult>(`/api/v1/balancetes/${balanceteId}/auditoria/rodar`, { method: 'POST', token }),
    listBalanceteFindings: (balanceteId: string, params: { severity?: FindingSeverity; status?: FindingStatus } = {}) => {
      const q = new URLSearchParams()
      if (params.severity) q.set('severity', params.severity)
      if (params.status) q.set('status', params.status)
      const suffix = q.toString() ? `?${q}` : ''
      return fetchJSON<{ data: BalanceteFinding[] }>(`/api/v1/balancetes/${balanceteId}/findings${suffix}`, { token })
    },
    getBalanceteFindingsResumo: (balanceteId: string) =>
      fetchJSON<BalanceteResumoResponse>(`/api/v1/balancetes/${balanceteId}/findings/resumo`, { token }),
    resolverBalanceteFinding: (findingId: string, dto: ResolverFindingDTO) =>
      fetchJSON<BalanceteFinding>(`/api/v1/findings/${findingId}`, { method: 'PATCH', body: JSON.stringify(dto), token }),
    liquidarBalancete: (balanceteId: string) =>
      fetchJSON<Balancete>(`/api/v1/balancetes/${balanceteId}/liquidar`, { method: 'POST', token }),
    listDepartamentos: () =>
...
// ─── Auditoria do balancete (migration 046) ───────────────────────────────
export type FindingSeverity = 'warning' | 'danger'
export type FindingStatus = 'aberto' | 'justificado' | 'corrigido' | 'arquivado'
...
export interface BalanceteFinding {
  id: string
  org_id: string
  balancete_id: string
...
export interface BalanceteFindingsResumo {
  total: number
  abertos: number
  danger: number
  warnings: number
...
export interface BalanceteResumoResponse {
  resumo: BalanceteFindingsResumo
  pode_liquidar: boolean
}

export interface RodarAuditoriaResult {
  balancete_id: string
  run_token: string
...
export interface ResolverFindingDTO {
